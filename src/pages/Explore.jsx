...
import ChangeIndicator from '../components/crypto/ChangeIndicator.jsx';
import {
    FALLBACK_GHS,
    COIN_META,
    COIN_META_MAP,
...
const ASSET_FILTERS = ASSET_FILTER_OPTIONS.map((item) => {
    const iconMap = {
        all: <GlobeIcon />,
        tradeable: <TradeIcon />,
        new: <NewIcon />,
    ...
    return {
        value: item.value,
        label: item.label,
        icon: iconMap[item.iconKey] || <GlobeIcon />,
    };
});
...
const MarketStatCard = ({ label, value, change, statId }) => {
    ...
    return (
        <Link to="/market-stats" className="flex-1 min-w-60 h-35 bg-gray-5 rounded-lg p-4 hover:shadow-elevation-1 transition-shadow duration-200 cursor-pointer overflow-hidden relative border border-transparent hover:border-gray-10 block">
...
const generateSparklineData = (changePct, coinId = '', W = 120, H = 40) => {
    const pts = 24;
...
const TableLineChart = ({ changePct, coinId }) => {
    ...
    const stroke = (changePct ?? 0) < 0 ? '#CF202F' : '#098551';
                            ...
                            <section className="pb-8 border-b border-gray-10 mb-8">
                                ...
                                {coins.length > 0 && (
                                    <div className="text-body text-gray-60 mb-2 max-w-4xl">
                                        <p className="mb-4">
                                            {EXPLORE_MARKET_COPY.marketSummaryStart} {(btcCoin?.price_change_percentage_24h ?? 0) > 0 ? 'growing' : 'shrinking'} {EXPLORE_MARKET_COPY.marketSummaryEnd} {fmtCompact(totalMktCap, currLabel)}, {EXPLORE_MARKET_COPY.marketSummaryTail}
                                        </p>
                                        {showMoreStats && (
                                            <p>{EXPLORE_MARKET_COPY.marketDetails}</p>
                                        )}
                                    </div>
                                )}
                                <button
                                ...
                                <div className="py-8 md:py-12 px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-8">
                                    ...
                                    <div className="shrink-0">
                                        <img
                                            ...
                                            className="w-60 md:w-[280px] h-auto"
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <aside className="w-full lg:w-[320px] shrink-0 flex flex-col gap-6 lg:pl-8 lg:border-l lg:border-gray-10">
                            ...
                            <div className="bg-blue-60 rounded-2xl p-5 relative overflow-hidden min-h-[160px]">
                                <div className="relative z-10 w-[65%] shrink-0">
                                    ...
                                    <p className="text-[15px] leading-5 text-white mb-5">Create your account today</p>
                                    <Link to="/signup">
                                ...
                                <div className="flex gap-3 overflow-x-auto pb-2">
                                    {loading
                                        ? Array.from({ length: 2 }).map((_, i) => (
                                            <div key={i} className="flex-1 min-w-30 bg-gray-5 rounded-xl p-4 h-30 animate-pulse" />
                                        ))
                            ...
                            <div>
                                ...
                                <div className="flex gap-3 overflow-x-auto pb-2">
                                    {loading
                                        ? Array.from({ length: 2 }).map((_, i) => (
                                            <div key={i} className="flex-1 min-w-35 bg-gray-5 rounded-xl p-4 h-30 animate-pulse" />
                                        ))
                                        : newCoins.map((coin) => (

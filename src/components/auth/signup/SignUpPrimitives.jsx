...
export const SignUpShell = ({ children, maxW = '440px' }) => (
...
export const BlueButton = ({ children, onClick, disabled, type = 'button' }) => (
    <button
        type={type}
        onClick={onClick}
        disabled={disabled}
        className={`w-full h-14 rounded-full font-semibold text-[0.9375rem] transition-colors ${disabled
...
export const DarkSelect = ({ label, hint, value, onChange, children }) => (
    <div className="mb-4">
        {label && <label className="block text-[0.875rem] font-semibold text-white mb-1.5">{label}</label>}
